...
def load_flts(
    ...
    **kwargs
    ...
    fnam = fnam.rsplit("_", 1)[0]  # Remove the last section (_00001) of the fnam search key
    fnams = glob.glob(fnam+"*.flt")

    # Print file names only when debugging
    if debug:
        print(f"Key used for searching: {fnam}")  # Confirm correct search key has been used
        for f in fnams: print(f)  # Print out the names of the discovered
    else: pass
    '''
    # 2023-12-05: The tables of data were appended to a list.
    # 2023-12-05: This meant the filename was not carried forward to subsequent processes.
    # 2023-12-05: Suggest using a dictionary structure instead, with file names as the keys.

    # Load and append files to list
    obj_list = []  # Empty list to store data in
    #obj = columnfile()
    for i in range(len(fnams)):
        # Load file using ImageD11's columnfile() function
        obj = columnfile(fnams[i])
        obj_list.append(obj)  # Append to list

        # Extract values
        x = -obj.dety
        y = obj.detz
        z = obj.sum_intensity
        n = obj.Number_of_pixels


        # Plot graphs only if debug is True
        if debug:
            # Plot the location of the peaks
            plt.scatter(-obj.dety, obj.detz,1,c=(obj.sum_intensity))
            plt.colorbar()
            plt.title(fnams[i])
            plt.show()

            # Plot peaks based on number of pixels in peak and intensity
            plt.scatter((obj.Number_of_pixels),(obj.sum_intensity))
            plt.title(fnams[i])
            plt.show()
    ...
    # Load and append files to dictionary
    obj_dict = {}  # Empty list to store data in
    #obj = columnfile()
    for i in range(len(fnams)):
        # Load file using ImageD11's columnfile() function
        fnam = fnams[i]
        obj = columnfile(fnams[i])
        obj_dict[fnam] = obj  # Append to dictionary
...
def make_im_from_flts(
    ...
    **kwargs):
    ...
    if setting_class is None:
        settings_for_fit = initiate(setting_file,
                                    inputs=inputs,
                                    report=True)
    else:
        settings_for_fit = setting_class
    ...
    peaks_im = ma.zeros(data_class.intensity.shape)  # Empty array the same size as data

    # 2023-05-12: Checked FLT files. They don't contain polar coordinate information

    # 2023-05-12: Test cpf.input_types.DioptasFunctions.DioptasDetector
    # 2023-05-12: Extract two-theta and azimuth values for each each pixel
    two_theta = data_class.get_two_theta()
    azimuth = data_class.get_azimuth()

    # Print messages when debugging
    if debug:
        print(f"Shape of two_theta array: {np.shape(two_theta)}", "\n",
              f"Shape of azimuth array: {np.shape(azimuth)}")
    else: pass

    # Get plotting ranges (x, y, two-theta, azimuth)
    x_range = (0, np.shape(peaks_im)[1])  # x axis = array columns
    y_range = (0, np.shape(peaks_im)[0])  # y axis = array rows
    tth_range = (np.min(two_theta), np.max(two_theta))
    az_range = (np.min(azimuth), np.max(azimuth))

    # Load in list of tables
    pks = load_flts(setting_file=setting_file,
                    setting_class=settings_for_fit,
                    data_class=data_class,
                    debug=debug)

    '''
    # 2023-12-05: Use this for loop condition for list object
    for i in range(len(pks)):
        # Test for one loop only if debugging
        if debug:
            if not i == 0:
                print(f"Plotting only iteration {0 + 1} for debugging")
                continue
            else: pass
        else: pass

        # Extract x, y, and intensity values
        x = -pks[i].dety  # x-axis, cols
        y = pks[i].detz  # y-axis, rows
        z = pks[i].sum_intensity  # Integrated intensity of peak

        # Print messages when debugging
        if debug:
            print(f"Shape of x: {np.shape(x)}", "\n",
                  f"Shape of y: {np.shape(y)}")  # Checked dimensions of arrays. List of coordinates
            print(f"x is a {type(x)}", "\n",
                  f"y is a {type(y)}")  # Check type. 1D Numpy array.
        else: pass

        # Iterate through saved peaks and find corresponding azimuth and two-theta values
        az = []
        tth = []
        for j in range(len(x)):
            peaks_im[int(y[j]), int(x[j])] = z[j]  # Populate array with values

            # Find two-theta and azimuth values that correspond to given x and y coords
            # Save as NumPy array
            az = np.append(az, [azimuth[int(y[j]), int(x[j])]], axis=0)
            tth = np.append(tth, [two_theta[int(y[j]), int(x[j])]], axis=0)

        # Print messages when debugging
        if debug:
            print(f"Final shape of the azimuth array: {np.shape(az)}", "\n"
                  f"Final shape of the two theta array: {np.shape(tth)}")

        # Plot scatter plots
        # Cartesian (xy) coordinates, intensity
        plt.scatter(x, y, 1, z)
        plt.gca().set_box_aspect(len(y)/len(x))  # Set square pixel aspect ratio
        plt.colorbar()
        plt.title(f"Cartesian Plot for Threshold {i + 1}")
        ...
        # Polar coordinates, intensity
        plt.scatter(tth, az, 1, z)
        plt.gca().set_box_aspect(len(az)/len(tth))  # Set square pixel aspect ratio
        plt.colorbar()
        plt.title(f"Polar Plot for Threshold {i + 1}")
        plt.xlabel(r"2$\theta$ ($\degree$)")
        plt.xlim(*tth_range)
        plt.ylabel(r"Azimuth ($\degree$)")
        plt.ylim(*az_range)
        plt.show()
    '''

    # 2023-12-05: Use this for loop condition for dict object
    keys = [key for key in pks.keys()]  # Load dictionary keys as list
    for i in range(len(keys)):
        ...
        k = keys[i]
        fnam = os.path.abspath(k).split("\\")[-1]  # Take only file stem name

        # Extract x, y, and intensity values
        x = -pks[k].dety  # x-axis, cols
        y = pks[k].detz  # y-axis, rows
        z = pks[k].sum_intensity  # Integrated intensity of peak
        ...
        plt.gca().set_box_aspect(len(y)/len(x))  # Set square pixel aspect ratio
        plt.colorbar()
        plt.title(f"Cartesian (x,y) plot of {fnam}")
        plt.xlabel("x (pixels)")
        plt.xlim(*x_range)
        plt.ylabel("y (pixels)")
        plt.ylim(*y_range)
        ...
        plt.gca().set_box_aspect(len(az)/len(tth))  # Set square pixel aspect ratio
        plt.colorbar()
        plt.title(f"Polar plot of {fnam}")
        plt.xlabel(r"2$\theta$ ($\degree$)")
        plt.xlim(*tth_range)
        plt.ylabel(r"Azimuth ($\degree$)")
        plt.ylim(*az_range)
