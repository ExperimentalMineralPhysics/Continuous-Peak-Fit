...
No surprise, this python module is much faster then the IRAF implementation, as it does not read/write every step to
disk.
Usage
=====
...
class cosmicsimage:
    ...
    def labelmask(self, verbose=None):
        ...
        if verbose:
            logger.info(" ".join(map(str, [("     Labeling mask pixels ...")])))
        ...
        centers = [
            [(tup[0].start + tup[0].stop) / 2.0, (tup[1].start + tup[1].stop) / 2.0]
        ...
        retdictlist = [
            {"name": "%i" % size, "x": center[0], "y": center[1]}
            for (size, center) in zip(sizes, centers)
        ]

        if verbose:
            logger.info(" ".join(map(str, [("     Labeling done")])))
    ...
    def clean(self, mask=None, verbose=None):
        ...
        # That's it.
        if verbose:
            logger.info(" ".join(map(str, [("   Cleaning done")])))
    ...
    def findsatstars(self, verbose=None):
        ...
        if verbose is None:
            verbose = self.verbose
        if verbose:
            logger.info(" ".join(map(str, [("     Detecting saturated stars ...")])))
        # DETECTION
        ...
        if verbose:
            logger.info(" ".join(map(str, [("     Mask of saturated stars done")])))
    ...
    def lacosmiciteration(self, verbose=None):
        ...
        if verbose:
            logger.info(" ".join(map(str, [("     Creating noise model ...")])))
        ...
        if self.satstars is not None:
            if verbose:
                logger.info(" ".join(map(str, [("     Masking saturated stars ...")])))
        ...
        growcosmics = np.cast["bool"](
            signal.convolve2d(
                np.cast["float32"](cosmics), growkernel, mode="same", boundary="symm"
            )
        )
...
def image_preprocess(data_as_class, settings_for_fit):
    """
    function to process the cosmics outside of fitpattern.
    ...
    """
    logger.info(" ".join(map(str, [("Remove Cosmics")])))
    # set defaults
    gain = 2.2
    ...
    for i in range(num):
        ...
        # logger.info(" ".join(map(str, [(asdf["nnew"], asdf["niter"])])))
        test.clean()
        msk = np.logical_or(
