    ...
    for j in range(settings_for_fit.datafile_number):
        ...
        for i in range(len(settings_for_fit.fit_orders)):
            ...
            if mode == "set-range":
                ...
                # if don't wanna close graphs remove close() close above three lines and uncomment line below
                # plt.show()
                ...
                # print(selection_arr)
                print("")
                print("Selected points, %s:" % peak_string(settings_for_fit.subfit_orders))
                print("[")
...
if __name__ == "__main__":
    ...
    print(settings_file)
    execute(inputs=None, debug=False, refine=True, save_all=True, iterations=1, setting_file=settings_file, parallel=False)
