            ...
            if not PreviousParams:
                ...
                if 'PeakPositionSelection' not in orders:
                    ...
                    for j in range(peeks):
                        ...
                        if np.min(tthguess[:,1]) < lims_tth[0] or np.max(tthguess[:,1]) > lims_tth[1]:
                ...
                for j in range(len(chunks)):
                    ...
                    if np.sum(~intens.flatten()[chunks[j]].mask) >= min_dat:
                        ...
                        for k in range(peeks):
                            ...
                            peaks.append({"height": [hguess], "d-space": [dguess], "width": [wguess], "profile": [pguess]})
                ...
                for j in range(peeks):
                    ...
                    arr_names = ['newHall','newd0','newWall','newPall']
                    arr_err_names = ['newHallErr','newd0Err','newWallErr','newPallErr']                   
            ...
            if refine or step!=10 or step!=15:
                ...
                for j in range(iterations):
                    ...
                    for k in range(peeks):
                        ...
                        comp_list = ['h', 'd', 'w'] # always need to iterate over d, height and width
                        comp_names = ['height','d-space','width','profile']
                        if 'profile_fixed' in orders['peak'][k]:
                            pfixed=1
                        else:
