...
class XYDetector:
    ...
    def plot_calibrated(
        ...
        if plot_as_image and y_axis != "intensity":
            ...
            # plot_x[plot_x.mask!=True]=0
            # plot_y[plot_y.mask!=True]=0
            
            merged = np.array([plot_x.flatten(), plot_y.flatten()]).T
            
            
            import matplotlib.tri as tri
            try:
                triang = tri.Triangulation(plot_x[plot_x.mask!=True].flatten(), plot_y[plot_y.mask!=True].flatten())
                
                
                
                
                
                plot_i = plot_i[plot_i.mask!=True].flatten()
                
                
                #triang.set_mask(mask)
            except:
                    
                triang = tri.Triangulation(plot_x.flatten(), plot_y.flatten())
                plot_i = plot_i.flatten()
                
            #print(triang.triangles)
            
            
            triang = tri.Triangulation(plot_x.data.flatten(), plot_y.data.flatten())
            corners = triang.triangles
            mask=[]
            for i in range(len(corners.flatten())-1):
                if any(plot_x.flatten()[corners[i,:]].mask == True):
                    mask.append(True)
                else:
                    mask.append(False)
            mask=np.array(mask)
            ...
            plot_i[np.isfinite(plot_i) == False] = 0
            ...
            mask = TriAnalyzer(triang).get_flat_tri_mask(min_circle_ratio=.5, rescale=True) 
            ...
            #centers = np.sum(merged[tri.simplices], axis=1, dtype='int')/3.0
            #colors = np.array([ (x-w/2.)**2 + (y-h/2.)**2 for x,y in centers])
            # colors = plot_i.flatten()
            ...
            the_plot = axis_plot.tricontourf(
                triang,
                plot_i,            
                mask=mask,
                # tri.simplices.copy(), 
