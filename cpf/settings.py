...
class settings:
    ...
    def validate_output_types(self, report=False):
        ...
        if missing:
            ...
            for i in range(len(missing)):
                settings.logger.warn(missing[i])
            settings.logger.warn("The issues listed above may prevent outputs being written correctly")
        else:
            settings.logger.warn("The output settings appear to be in order")
