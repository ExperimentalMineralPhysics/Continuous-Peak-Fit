...
def get_image_keys_new(
    ...
    for i in range(len(loops)):
        ...
        if itera["using"] == "value":
            ...
            keep, _ = IO.StartStopFilesToList(paramDict=itera)
    ...
    for i in range(len(keylist)):
        ...
        if not keylist[i] in df.keys():
            ...
            if itera["do"] == "sum":
                ...
                index_values, _ = IO.StartStopFilesToList(paramDict=itera)
    ...
    for j in range(len(label_list)): 
        ...
        if lb.lower() == "pos":
            ...
            try:
                ...
                if lb[-1] == "/":
                    label += IO.licit_filename(f"{lb[div[-2]+1:div[-1]]}{sep2}{ky}")
                else:
                    label += IO.licit_filename(f"{lb[div[-1]+1:]}{sep2}{ky}")
            except:
                err_str = (
