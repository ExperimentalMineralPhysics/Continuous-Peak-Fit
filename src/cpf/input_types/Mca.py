...
class McaFit:
    ...
    def __init__(self, mca=None):
        ...
        self.energy_offset = 0.0  # Energy calibration offset
        self.energy_slope = 0.0  # Energy calibration slope
        self.energy_flag = 1  # Energy flag
        #   0 = Fix energy calibration coefficients
        #   1 = Optimize energy calibration coefficients
        self.initial_fwhm_offset = 0.15  # FWHM offset = 150 eV
        self.initial_fwhm_slope = 0.0
        self.fwhm_offset = 0.0  # FWHM offset
        self.fwhm_slope = 0.0  # FWHM slope
        self.fwhm_flag = 1  # Fwhm flag
        #   0 = Fix FWHM coefficients
        #   1 = Optimize FWHM coefficients
        self.chi_exp = 0.0  # Exponent of chi
        ...
        self.tolerance = 1.0e-4  # Convergence tolerance
        self.chisqr = 0.0  # Chi-squared on output
...
class McaPeak:
    ...
    def __init__(self):
        ...
        #   1 = Optimize FWHM
        #   2 = Fix FWHM to input value
        self.ampl_factor = 0.0  # Fixed amplitude ratio to previous peak
        #   0.  = Optimize amplitude of this peak
        #   >0. = Fix amplitude to this value relative
        #         to amplitude of previous free peak
        #  -1.0 = Fix amplitude at 0.0
        self.initial_energy = 0.0  # Peak energy
        self.energy = 0.0  # Peak energy
        self.initial_fwhm = 0.0  # Peak FWHM
        self.fwhm = 0.0  # Peak FWHM
        self.initial_ampl = 0.0  # Peak amplitude
        self.ampl = 0.0  # Peak amplitude
        self.area = 0.0  # Area of peak
        self.bgd = 0.0  # Background under peak
...
class McaCalibration:
    ...
    def __init__(self, offset=0.0, slope=1.0, quad=0.0, units="keV", two_theta=10.0):
        """
...
class Mca(object):
    ...
    def channel_to_d(self, channels):
        ...
        return 12.398 / (
            2.0 * e * math.sin(self.calibration.two_theta / 2.0 * math.pi / 180.0)
        )
    ...
    def energy_to_channel(self, energy, clip=0):
        ...
        if self.calibration.quad == 0.0:
            channel = (energy - self.calibration.offset) / self.calibration.slope
        else:
    ...
    def d_to_channel(self, d, clip=0):
        ...
        """
        e = 12.398 / (
            2.0 * d * math.sin(self.calibration.two_theta * math.pi / 180.0 / 2.0)
        )
    ...
    def write_file(self, file, netcdf=0):
        ...
        if netcdf != 0:
            write_netcdf_file(
                file, data, calibration, elapsed, presets, rois, environment
            )
        else:
            write_ascii_file(
                file, data, calibration, elapsed, presets, rois, environment
...
def read_ascii_file(file):
    ...
    while 1:
        ...
        value = str.strip(
            line[pos:]
        )  # line[pos:].strip #value = string.strip(line[pos:])
