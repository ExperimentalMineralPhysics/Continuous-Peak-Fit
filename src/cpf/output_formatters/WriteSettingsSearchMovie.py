...
def WriteOutput(
    ...
    **kwargs : TYPE
    ...
    pattern = settings_class.output_settings.get(
        "pattern", Requirements()[1]["pattern"]
    )
    search_parameter = settings_class.output_settings.get(
        "search_parameter", Requirements()[1]["search_parameter"]
    )
    search_over = settings_class.output_settings.get(
        "search_over", Requirements()[1]["search_over"]
    )
    ...
    if settings_class.image_number == 1:
        ...
        fls = glob.glob(
            f"./{settings_class.output_directory}/*scan*{search_parameter}*.json"
        )
        if len(fls) == 0:
    ...
    if not search_parameter or search_parameter == "":  # or not search_over:
        # get pattern name
        pattern_name = os.path.splitext(
            os.path.basename(settings_class.image_list[pattern])
        )[0]
        # use filenames for seaech paramter
        fls = glob.glob(
            f"./{settings_class.output_directory}/*{pattern_name}*scan*{search_parameter}*.json"
        )
        search_parameter = (
            os.path.splitext(os.path.basename(fls[0]))[0].split("__")[1].split("=")[1]
        )
    ...
    for srch in search:
        ...
        settings_class_reduce.file_label = (
            "scan=" + search_parameter + "__" + "value=" + str(srch)
        )
        ...
        settings_class_reduce._unmodified_self = (
            settings_class_reduce._validation_copy()
        )
    ...
    for i in range(len(ranges)):
        ...
        df_peak = df[(df["range_position"] == ranges[i])]
