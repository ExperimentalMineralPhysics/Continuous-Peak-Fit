...
def fourier_to_crystallographic(coefficients, SampleGeometry="3d", SampleDeformation="compression", correlation_coeffs=None, subpattern = 0, peak=0, debug=False, **kwargs):
    """
    ...
    **kwargs : TYPE
    ...
    if not isinstance(coefficients, list):
        raise ValueError(
            "The coefficients need to be a list of dictionaries."
        )
    ...
    out_dmaxerr = (coefficients[subpattern]["peak"][peak]["d-space_err"][0]**2 + out_dderr**2) ** (1/2)
    # differential min
    out_dmin = coefficients[subpattern]["peak"][peak]["d-space"][0] - out_dd
    out_dminerr = (coefficients[subpattern]["peak"][peak]["d-space_err"][0]**2 + out_dderr**2) ** (1/2)
    ...
    if SampleGeometry=="2d":
        ...
        out_d0err = (coefficients[subpattern]["peak"][peak]["d-space_err"][0] ** 2 +  (out_dderr/3) ** 2 ) ** (1/2)  
