    ...
    if len(SeriesConstraints(seriesOrder))==0: #isinstance(seriesOrder, list):
        ...
        for i in range(len(Series_Constraints)):
            ...
            if "fixed" in Series_Constraints[i].lower():
                ...
                value = tuple(map(float, (Series_Constraints[i].removeprefix("fixed").removeprefix("(").removesuffix(")").split(','))))
                expression = None
                ...
                parts = Series_Constraints[i].replace("greater","equal").replace("less", "equal").split("equal")
                ...
                if "all" in parts[0]:
                    ...
                    split_orders = list(map(int, parts[0].replace("order", "").replace("st", ",").replace("nd", ",").replace("rd", ",").replace("th", ",").removesuffix(",").split(",") ))
                    positionsFixed = []
                    ...
                    for j in range(len(split_positions)):
                        if "-" in split_positions[j]:
                            ...
                            split_positions[j] = list(range(split_positions[j][0],split_positions[j][1],1))
                    #make sure the list is flat.
                    positionsFixed = [item for items in split_positions for item in items]
                
                # from second  bit of parts determine what these are constrained to. 
                parts[1] = list(map(int, (parts[1].replace("peaks", "").replace("peak", ""))))
                if len(parts[1]) != 1:
            ...
            elif ("delta" in Series_Constraints[i] or 
                        "greater" in Series_Constraints[i] or 
                        "less" in Series_Constraints[i]):
                raise NotImplementedError("non equal constrains on parameters are not implemented")                    
