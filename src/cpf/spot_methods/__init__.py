...
The modules all require "spots" in their filename.
This allows the addition of new modules by just adding them to the directory.

The modules loaded from each spots*.py file must contain a class called *Class. 
...
for _module_path in _os.listdir(_os.path.dirname(__file__)):
    if (
        ...
        _module: _ModuleType = _import_module(f"cpf.spot_methods.{_output_module}")
