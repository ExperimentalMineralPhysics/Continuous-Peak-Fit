...
if 0:
    ...
    num_ang = 500
    small = 1
    d0 = np.random.default_rng().uniform(.5,7,num_ang) # Angstrons
    Q = np.random.default_rng().uniform(0,.4,num_ang)#*d0 # as per Singh et al., 1998
    ...
    SampleGeometry = (2 + np.round(np.random.uniform(size=num_ang))).astype(int) #dimensions
...
for i in range(len(orientation)):
    ...
    # Singh et al 1998.
    d_azm1 = d0[i] * (1+ Q[i]*pos_neg *(1-SampleGeometry[i]*(np.cos(np.deg2rad(azm-orientation[i])))**2))
    ...
    Singh_coef = cfc.fourier_to_crystallographic(new_params, SampleGeometry=f"{SampleGeometry[i]:1d}d", SampleDeformation=SampleDeformation[i], debug=debug)

    if 0:
        print(f'Centroid:    Initial value {d0[i]:.3f}; recovered value {Singh_coef["dp"]:.3f}; difference {(d0[i]-Singh_coef["dp"])/d0[i]*100:4.4g}%')
        print(f'Q:           Initial value {Q[i]:.3f}; recovered value {Singh_coef["differential"]:.3f}; difference {(Q[0]-Singh_coef["differential"])/Q[i]*100:4.4g}%')
        print(f'orientation: Initial value {orientation[i]:.3f}; recovered value {Singh_coef["orientation"]:.3f}; difference {(orientation[i]-Singh_coef["orientation"])/360*100:4.4g}% of circle')
        print(f'x:           Initial value {x0[i]:.4g}; recovered value {Singh_coef["x0"]:.3g}; difference {(x0[i]-Singh_coef["x0"])/x0[i]*100:4.4g}%')
        print(f'y:           Initial value {y0[i]:.4g}; recovered value {Singh_coef["y0"]:.3g}; difference {(y0[i]-Singh_coef["y0"])/y0[i]*100:4.4g}%')
    ...
    second_order_corefs.append([fout.params.valuesdict()["peak_0_d3"], fout.params.valuesdict()["peak_0_d4"]])
...
if 1:
    ...
    ax.plot([np.min(angles[:,0]),np.max(angles[:,0])], [np.min(angles[:,0]),np.max(angles[:,0])], ':', color='black')
    ...
    angles2=angles
    angles2[angles[:,1]-angles[:,0]>90,1] = angles2[angles[:,1]-angles[:,0]>90,1]-180
    angles2[angles[:,1]-angles[:,0]<-90,1] = angles2[angles[:,1]-angles[:,0]<-90,1]+180
    fig, ax = plt.subplots()
    ...
    fig, ax = plt.subplots()
    ax.scatter(np.sqrt(xs[:,0]**2+ys[:,0]**2), np.abs(angles2[:,1]-angles2[:,0]), s=2, c=Qs[:,0])
    ax.set_title("Delta orientation angle")
    ...
    ax.plot([np.min(d0s[:,0]),np.max(d0s[:,0])], [np.min(d0s[:,0]),np.max(d0s[:,0])], ':', color='black')
    ax.set_title("d0")
    ...
    #plot differneces 
    fig, ax = plt.subplots()
    ax.scatter(d0s[:,0], (d0s[:,1]-d0s[:,0])/d0s[:,0]*100, s=2, c=np.sqrt(xs[:,0]**2+ys[:,0]**2))
    ...
    fig, ax = plt.subplots()
    ax.scatter(tths[:,0], np.abs(d0s[:,1]-d0s[:,0]), s=2, c=np.sqrt(xs[:,0]**2+ys[:,0]**2))
    ax.set_title("Delta d0")
    ...
    #ax.plot(azm, r, 'r')
    if 1:
        SC = ax.scatter(Qs[:,0], Qs[:,1], s=2, c=np.sqrt(xs[:,0]**2+ys[:,0]**2))
        fig.colorbar(SC, ax=ax, label="sqrt(x0^2 + y0^2)")
    else:
    ...
    fig, ax = plt.subplots()
    SC = ax.scatter(Qs[:,0], (Qs[:,1]-Qs[:,0]), s=2, c=np.sqrt(xs[:,0]**2+ys[:,0]**2)/tths[:,0])
    # plot liness ot equal magnitude.
    ax.plot([0,  np.min([np.max(Qs[:,0]),np.max(Qs[:,1]-Qs[:,0])])],  [0, np.min([np.max(Qs[:,0]),np.max(Qs[:,1]-Qs[:,0])])], ':', color='black')
    ax.plot([0, np.min([np.max(Qs[:,0]),np.max(Qs[:,1]-Qs[:,0])])],  [0,-np.min([np.max(Qs[:,0]),np.max(Qs[:,1]-Qs[:,0])])], ':', color='black')
    ...
    if debug:       
        ...
        fig, ax = plt.subplots()
        #ax.plot(azm, r, 'r')
        SC = ax.scatter(ys[:,0], ys[:,1]/d0s[:,1]**2*distance*wavelength, s=2, c=Qs[:,0])
        #ax.plot(xs[:,0], xs[:,1], '.', color='red')
        #ax.plot([np.min(xs[:,0]),np.max(xs[:,0])], [np.min(xs[:,0]),np.max(xs[:,0])], ':', color='black')
        ax.set_title("y0")
        ax.set_xlabel("Known y0s")
        ...
        fig, ax = plt.subplots()
        #ax.plot(azm, r, 'r')
        SC = ax.scatter(ys[:,0], (ys[:,1]/d0s[:,1]**2*distance*wavelength-ys[:,0])/ys[:,0], s=2, c=Qs[:,0])
        #ax.plot(xs[:,0], xs[:,1], '.', color='red')
        #ax.plot([np.min(xs[:,0]),np.max(xs[:,0])], [np.min(xs[:,0]),np.max(xs[:,0])], ':', color='black')
        ax.set_title("y0")
        ax.set_xlabel("Known y0s")
